...
\begin{document}
	...
	\section*{Homework 10.2: \\ Solve by Square Roots; Completing the Square Setup}
		...
		\subsection*{Part A: Solve Using Square Roots}
			...
			Solve each equation. Simplify your answer as much as possible and give exact answers (leave in square root form).\\\\
		...
		\subsection*{Part C: Complete the Square (Setup Only)}
			...
			Then write the perfect square trinomial in its factored form.\\\\
		...
		\subsection*{Part D: Word Problem}
			...
			A square picture frame has an area of 81 square inches.\\
            ...
            Solve for $s$ using square roots. Give an exact answer.
	...
	\section*{Unit 10 Quiz: \\ Quadratic Graphs \& Completing the Square}
		...
		\subsection*{Section 1: Key Features of Quadratics}
			...
			For the function $y = x^2 - 4x + 3$, find all key features AND sketch the graph.

			\vspace{3cm}

			Direction: \underline{\hspace{3in}}

			\vspace{0.5cm}

			$y$-intercept: \underline{\hspace{3in}}

			\vspace{0.5cm}

			Axis of symmetry: \underline{\hspace{3in}}

			\vspace{0.5cm}

			Vertex: \underline{\hspace{3in}}

			\vspace{0.5cm}

			$x$-intercept(s): \underline{\hspace{3in}}
			...
			\begin{tikzpicture}[scale=0.8]
				% Grid
				\draw[step=0.5cm, gray!30, very thin] (-2,-4) grid (6,6);
				% Axes
				\draw[thick,->] (-2,0) -- (6,0) node[right] {$x$};
